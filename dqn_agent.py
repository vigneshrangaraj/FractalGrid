...
class Actor(nn.Module):
    def __init__(self, state_size, continuous_action_size, discrete_action_size, hidden_size=256):
        ...
        self.tanh = nn.Sigmoid()  # For continuous actions between [-1, 1]
        self.discrete_output = nn.Linear(hidden_size, discrete_action_size)
    ...
    def forward(self, state):
        ...
        output = self.continuous_output(x)
        continuous_action = self.tanh(self.continuous_output(x))  # Continuous action
        discrete_action = self.sigmoid(self.discrete_output(x))   # Discrete action
...
class Critic(nn.Module):
    def __init__(self, state_size, continuous_action_size, discrete_action_size, hidden_size=256):
        ...
        self.fc1 = nn.Linear(state_size + continuous_action_size + discrete_action_size, hidden_size)
        self.ln1 = nn.LayerNorm(hidden_size)  # Use LayerNorm instead of BatchNorm
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        self.ln2 = nn.LayerNorm(hidden_size)  # Use LayerNorm instead of BatchNorm
    ...
    def forward(self, state, continuous_action, discrete_action):
        ...
        x = torch.cat([state, continuous_action, discrete_action], dim=1)
...
class ActorCriticAgent:
    def __init__(self, state_size, continuous_action_size, discrete_action_size, gamma=0.99, tau=0.001,
                 learning_rate=0.0001, batch_size=64, memory_size=10000, epsilon=1.0, epsilon_min=0.01, epsilon_decay=0.9995):
        ...
        self.actor = Actor(state_size, continuous_action_size, discrete_action_size).to(device)
        self.actor_target = Actor(state_size, continuous_action_size, discrete_action_size).to(device)
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=self.learning_rate)

        # Critic networks
        self.critic = Critic(state_size, continuous_action_size, discrete_action_size).to(device)
        self.critic_target = Critic(state_size, continuous_action_size, discrete_action_size).to(device)
    ...
    def select_action(self, state, test_mode=False, add_noise=True):
        ...
        continuous_action, discrete_action = self.actor(state_tensor)
        ...
        discrete_action = discrete_action.cpu().detach().numpy()[0]
        ...
        if test_mode:
            ...
            discrete_action = np.round(discrete_action).astype(int)  # Exploit discrete actions (0 or 1)
        else:
            ...
            if np.random.rand() <= self.epsilon:
                ...
                discrete_action = np.random.randint(2, size=self.discrete_action_size)  # Random binary discrete action
            else:
                ...
                # Threshold discrete actions (0 or 1 for binary switches)
    ...
    def replay(self):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            next_continuous_action, next_discrete_action = self.actor_target(
            ...
            next_Q = self.critic_target(torch.FloatTensor(next_state).unsqueeze(0).to(device), next_continuous_action,
                                        next_discrete_action)
            target_Q = reward + (1 - done) * self.gamma * next_Q
            current_Q = self.critic(torch.FloatTensor(state).unsqueeze(0).to(device),
                                    torch.FloatTensor(action[0]).unsqueeze(0).to(device),
                                    torch.FloatTensor(action[1]).unsqueeze(0).to(device))
            ...
            predicted_continuous_action, predicted_discrete_action = self.actor(
                torch.FloatTensor(state).unsqueeze(0).to(device))
            actor_loss = -self.critic(torch.FloatTensor(state).unsqueeze(0).to(device), predicted_continuous_action,
                                      predicted_discrete_action).mean()
