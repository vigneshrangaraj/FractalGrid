...
def plot_total_grid_power_and_pv_dispatch(time_steps, total_grid_power_data, total_pv_dispatch_data):
    ...
    plt.plot(time_steps, total_pv_dispatch_data, label='Total Net Load')
...
def plot_battery_soc(time_steps, charge_discharge_data, soc_data):
    ...
    fig, axs = plt.subplots(rows, cols, figsize=(30, 20 * rows))  # Create grid layout
...
def plot_switching_schedule(time_steps, switching_data):
    plt.figure(figsize=(30, 20))
    # iterate through the switching data and retain only last 161 records in the value. Remember switching_data is a list of lists
...
def plot_power_transfer(time_steps, power_transfer_data):
    ...
    for i, power_transfer in enumerate(power_transfer_data):
        ...
        ax.plot(time_steps, power_transfer[-161:], label=f'Microgrid {i + 1} Transfer')
...
if __name__ == "__main__":
    ...
    env = FractalGrid(num_microgrids=3)  # Adjust initialization as needed
